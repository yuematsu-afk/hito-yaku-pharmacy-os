// src/lib/matching/scorePharmacist.ts
import type {
  Patient,
  Pharmacist,
  Pharmacy,
...
// /diagnosis・/result・/pharmacists で共通で使う相談スタイルキー
...
/**
 * 相談スタイルごとのラベル・説明
 * - UI 表示やスコア理由メッセージの両方で利用
 */
export const CARE_STYLE_INFO: Record<
  CareStyleKey,
  {
    label: string;
    description: string;
 ...
 * △：状況によっては合う
 */
export const TYPE_STYLE_MATCH: Record<
  PatientType,
  Record<CareStyleKey, "◎" | "◯" | "△">
> = {
  A: {
 ...
 */
export function getMainSymptom(
...
/**
 * 患者情報 ＋ 顧問薬剤師タイプ ＋ 薬剤師・薬局情報から
 * 0〜100 点のスコアと、主な理由（最大3つ）を返す
 */
