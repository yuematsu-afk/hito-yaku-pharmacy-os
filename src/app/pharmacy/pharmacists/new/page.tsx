// src/app/pharmacy/pharmacists/new/page.tsx
"use client";

import { Suspense } from "react"; // ★ 本番環境で必要。ローカルでは使わない
...
// 1) 外側：Suspense でラップするだけ
export default function PharmacyPharmacistNewPage() {
  return (
    <Suspense
      fallback={
    ...
    >
      <InnerNewPharmacistPage />
    </Suspense>
  );
}

function InnerNewPharmacistPage() {
  ...
  // 初期データ取得：法人ID＋stores一覧
  useEffect(() => {
    if (!user) return;

    if (!isPharmacyCompany && !isAdmin) {
      setError("薬局法人アカウントまたは管理者のみ薬剤師を登録できます。");
      setStatus("error");
      return;
    }
    ...
    const run = async () => {
      ...
      try {
        ...
        // 薬局法人ユーザーの場合は、自分の pharmacy_company_id を取得
        ...
        // stores 一覧（法人ユーザー → 自社店舗のみ / admin → 全店舗）
        ...
        setPharmacyCompanyId(ownedPharmacyId);
        setStores(storesData ?? []);

        if (!storesData || storesData.length === 0) {
          setError(
            "この薬局法人に紐づく店舗がまだ登録されていません。先に店舗を登録してください。"
          );
          setStatus("error");
          return;
        }

        // クエリからプリセット用 storeId を取得
        const storeIdFromQuery = searchParams.get("storeId");

        const defaultStoreId =
          storeIdFromQuery &&
          storesData.some((s) => s.id === storeIdFromQuery)
            ? storeIdFromQuery
            : storesData[0].id;

        const initialForm: FormState = {
          name: "",
          belongs_store_id: defaultStoreId,
          visibility: "members",
          access_scope: "public",
          short_message: "",
          languagesText: "",
        ...
        console.error("[pharmacy/pharmacists/new] init error", e);
    ...
    void run();
  }, [user, isPharmacyCompany, isAdmin, searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const selectedStore =
        stores.find((s) => s.id === form.belongs_store_id) ?? null;

      if (!selectedStore) {
        throw new Error(
          "選択された所属店舗が見つかりませんでした。ページを再読み込みしてから再度お試しください。"
        );
      }

      // 法人IDは「選択された店舗の pharmacy_id → なければログイン中の法人ID」
      const belongsPharmacyId: string | null =
        selectedStore.pharmacy_id ?? pharmacyCompanyId ?? null;
      ...
      const insertPayload = {
        ...
        // 店舗への直接紐づき
        belongs_store_id: selectedStore.id,
      ...
      if (insertError) {
        console.error("[pharmacy/pharmacists/new] insert error", insertError);
        throw new Error(
          "薬剤師の登録に失敗しました。入力内容を確認のうえ、時間をおいて再度お試しください。"
        );
      }

      if (!data?.id) {
        throw new Error(
          "薬剤師の登録は完了しましたが、詳細画面に遷移できませんでした。"
        );
      }

      router.push(`/pharmacy/pharmacists/${data.id}`);
    } catch (e: any) {
      console.error("[pharmacy/pharmacists/new] unexpected error", e);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-6">
      ...
      {status === "error" && stores.length === 0 && (
        <AppCard className="space-y-2 border-amber-200 bg-amber-50/70 text-xs text-amber-900">
          <p>
            この薬局法人に紐づく店舗がまだ登録されていないため、薬剤師を登録できません。
          </p>
          <p className="text-[11px]">
            先に「店舗管理」から本部・店舗を登録してから、再度この画面を開いてください。
          </p>
        </AppCard>
      )}
      ...
      {form && (
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-col gap-1">
                    <select
                      ...
                      className="w-full rounded-md border border-slate-300 bg白 px-2 py-1.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200"
                    >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateForm({ languagesText: e.target.value })
                  }
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  className="mt-1 h-28 w-full rounded-md border border-slate-300 bg-white px-2 py-1.5 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200"
                >
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateForm({ booking_url: e.target.value })
                  }
          ...
          <div className="flex justify-end">
            <AppButton type="submit" size="sm" disabled={disabled}>
              {status === "submitting" ? (
                <>
